...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.alexnainer.t3code.personal.dev");
      assert.equal(environment.linuxWmClass, "t3code-personal-dev");
      assert.equal(
        environment.linuxDesktopEntryName,
        "com.alexnainer.T3Code.Personal.Development.desktop",
      );
      assert.deepEqual(
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "com.alexnainer.T3Code.Personal.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.t3-personal/dev");
