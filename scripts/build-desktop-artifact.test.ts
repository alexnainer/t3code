...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("keeps the personal desktop packaging name across release channels", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code (Personal)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Code (Personal)");
  });
  ...
  it("installs optional native dependencies for the target desktop architecture", () => {
    ...
    assert.deepStrictEqual(resolveStageInstallArgs(undefined), ["install", "--prod"]);
    assert.deepStrictEqual(resolveStageInstallArgs(" https://npm.example.test/ "), [
      "install",
      "--prod",
      "--",
  ...
  it("uses a distinct staged package name for the personal Windows install directory", () => {
    assert.equal(DESKTOP_PACKAGE_NAME, "t3code-personal");
  });
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.alexnainer.t3code.personal",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.alexnainer.t3code.personal</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.alexnainer.t3code.personal");
